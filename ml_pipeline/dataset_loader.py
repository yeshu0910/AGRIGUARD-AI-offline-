...
class DatasetLoader:
    ...
    def discover_classes(self) -> List[str]:
        ...
        class_dirs = [
            d for d in self.dataset_dir.iterdir() if d.is_dir() and not d.name.startswith(".")
        ]
    ...
    def create_data_generators(self) -> Tuple[ImageDataGenerator, ImageDataGenerator]:
        ...
        val_datagen = ImageDataGenerator(
            rescale=1.0 / 255, validation_split=self.config.training.VAL_SPLIT
        )
        ...
        logger.info(
            f"  - Random brightness: ±{self.config.augmentation.RANDOM_BRIGHTNESS * 100:.0f}%"
        )

        return train_datagen, val_datagen

    def create_dataset(
        self, datagen: ImageDataGenerator, subset: str = "training"
...
def count_dataset_samples(dataset_dir: Path) -> Dict[str, int]:
    ...
    return {"total": total_images, "train": train_count, "val": val_count}
