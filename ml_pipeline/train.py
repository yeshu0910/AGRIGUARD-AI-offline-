...
from ml_pipeline.utils import logger, save_json, Timer, print_system_info
...
class Trainer:
    ...
    def setup(self) -> None:
        ...
        self.model_wrapper = create_model(
            num_classes=num_classes, input_shape=self.config.model.INPUT_SHAPE
        )
    ...
    def train(self) -> None:
        ...
        with Timer("Complete training"):
            self.history_initial, self.history_fine_tune = self.model_wrapper.train(
                self.train_generator, self.val_generator, class_weight=class_weight
            )
    ...
    def _plot_training_history(self) -> None:
        ...
        if "top_5_accuracy" in history:
            ...
            axes[1, 0].plot(
                history["val_top_5_accuracy"], label="Validation Top-5 Acc", linewidth=2
            )
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Train AgriGuard AI crop disease detection model")
    parser.add_argument(
        "--dataset-dir",
        type=Path,
        default=None,
